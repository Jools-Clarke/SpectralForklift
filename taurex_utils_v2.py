...
def full_contribution_array(mols, X_mols, Rs, Ts, Rp, Mp, Tp):
    ...
    FM = update_forward_model(FM,  mols, X_mols)
    res = FM.model()
    o = bin_forward_model(res[0],
                          res[1],
                          ariel_wn_grid,
                          ariel_wnwidth)


    res_c = FM.model_full_contrib()

    for i in range(len(res_c[1]['Absorption'])):
        species = res_c[1]['Absorption'][i][0]
        species_data = res_c[1]['Absorption'][i][1:]
        res_c_dic[species] = [res_c[0], species_data[0]]

        o_c_dic[species] = bin_forward_model(res_c_dic[species][0],
                                            res_c_dic[species][1],
                                            ariel_wn_grid,
                                            ariel_wnwidth)
